...
| 坐标 | 内容 | 源码仓库 |
|---|---|---|
| `io.github.zhihulite:luajvm-core` | Lua 5.5.1 的纯 Java 移植：VM、编译器、标准库、Java 绑定层。**桌面/JVM 用**——直接使用 Java 21 API，发布前强制跑官方/Lua/Java API 测试套件 | [zhihulite/luajvm](https://github.com/zhihulite/luajvm/luajvm-core) |
| `io.github.zhihulite:luajvm-core-android` | luajvm-core 的 **Android 兼容版**：core 源码直编 + 构建期把超 minSdk 24 的 API 调用重写为 `JavaApiCompat` 等价实现；发布前强制跑 Animal Sniffer **API 24 签名核对**（重写后的 jar 逐引用核对，超版本引用直接构建失败）。Android 宿主不要直接引 core，经 `luajvm-android` 的传递依赖自动获得本包 | 同上（机制见 `docs/android-compat.md`） |
| `io.github.zhihulite:luajvm-android` | luajvm 的 Android 平台层，以 `api` 依赖 core-android（消费方零感知），发布前强制跑 `lintRelease` | 同上 |
...
```groovy
dependencies {
    // luajvm-android 已传递依赖 luajvm-core-android（core 的 Android 兼容版），
    // 不要再单独声明 core / core-android
